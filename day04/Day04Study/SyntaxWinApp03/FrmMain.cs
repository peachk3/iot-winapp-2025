namespace SyntaxWinApp03
{
    public partial class FrmMain : Form
    {
        ...
        {
            // RtbNote.SelectionColor = Color.Red; // 선택된 텍스트 색상 변경
            if (RtbNote.SelectionLength > 0)
            {
                if (DlgColor.ShowDialog() == DialogResult.OK)
                {
                    RtbNote.SelectionColor = DlgColor.Color; // 선택된 텍스트 색상 변경
                }
            }
        }
        ...
        private void BtnBold_Click(object sender, EventArgs e)
        {
            ...
            if (currFont.Bold)
            {
                newStyle = currFont.Style & ~FontStyle.Bold; // 현재 Bold 스타일이 적용되어 있으면 제거
            }
            else
            {
                newStyle = currFont.Style | FontStyle.Bold; // 현재 Bold 스타일이 적용되어 있지 않으면 추가
            }
            RtbNote.SelectionFont = new Font(currFont, newStyle); // 선택된 텍스트를 Bold로 변경
        }
    }
}
